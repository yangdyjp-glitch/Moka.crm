...
如果目标电脑里的本地代码库是一个月前的旧版本，可以先手动把本文件复制到目标电脑项目根目录，然后在目标电脑的 Codex 里直接说：

```text
请先阅读项目根目录的 CODEX.md。这个本地代码库大约是一个月前的旧版本，请先从 GitHub main 同步到最新代码，再按 CODEX.md 的规则理解项目并进行后续修改。同步前先检查本地是否有未提交改动，不要覆盖用户改动。
```

如果目标电脑本地没有重要改动，可以同步远端主分支：

```text
git pull origin main
```

如果有本地改动，必须先保护本地改动，再同步。只要 `CODEX.md` 已复制过去，并明确告诉那边 Codex“先同步 main”，它就能理解项目、更新到最新代码库，然后继续修改。
